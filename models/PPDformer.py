...
class NoiseCancellation(nn.Module):
    def __init__(self, filter_order=5, mu=0.01, window_size=5, batch_size=32, c_in=21, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")):
        super().__init__()
        self.device = device
        self.filter_order = filter_order
...
class ComplexTensorProcessor(nn.Module):
    def __init__(self, d_model, configs):
        ...
        # Use provided configs.attention if available, otherwise fall back to output_attention
        self.attention = bool(getattr(configs, "attention", getattr(configs, "output_attention", False)))
...
def STFT_for_Period(x, k=3, n_fft=16, hop_length=8, win_length=16, window="hann"):
    ...
    for c in range(C):
        ...
        stft_result = torch.stft(single_channel_data, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window=window_tensor, return_complex=True)
