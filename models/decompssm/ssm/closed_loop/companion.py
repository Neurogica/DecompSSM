...
class ClosedLoopCompanionSSM(CompanionSSM):
    ...
    def __init__(self, lag: int = 1, horizon: int = 1, use_initial: bool = False, **kwargs):
        self.lag = lag
    ...
    def get_companion_matrix(self, p):
        ...
        return self.shift_matrix.to(p.device) + (oe.contract("h i, h j -> h j i", self.p_padding.to(p.device), p))
    ...
    def forward(self, u):
        ...
        if self.closed_loop:  # Compute closed-loop forecast
            ...
            k = self.norm(self.k, ord=self.norm_order) if self.norm_order > 0 else self.b
