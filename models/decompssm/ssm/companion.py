...
class CompanionSSM(SSM):
    ...
    def init_weights(self):
        ...
        self.shift_matrix = torch.zeros(self.n_kernels, self.kernel_dim, self.kernel_dim)
        self.shift_matrix[:, 1:, :-1] = torch.eye(self.kernel_dim - 1)
        self.p_padding = torch.zeros(*self._fp)
        self.p_padding[:, -1] = 1.0
    ...
    def matrix_power(self, l, c, b, p):
        ...
        A = self.shift_matrix.to(p.device) + (oe.contract("h i, h j -> h j i", self.p_padding.to(p.device), p))
