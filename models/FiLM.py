...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        configs.ratio = 0.5
        self.legts = nn.ModuleList([HiPPO_LegT(N=n, dt=1.0 / self.pred_len / i) for n in self.window_size for i in self.multiscale])
        self.spec_conv_1 = nn.ModuleList(
            [
                SpectralConv1d(in_channels=n, out_channels=n, seq_len=min(self.pred_len, self.seq_len), ratio=configs.ratio)
