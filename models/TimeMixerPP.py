...
from layers.TimeMixerPP import (
    AttentionLayer,
    ColAttention,
    DataEmbedding_wo_pos,
    FullAttention,
    Inception_Block_V1,
    Inception_Trans_Block_V1,
    Normalize,
    RowAttention,
)
...
class MultiScaleSeasonCross(nn.Module):
    def __init__(self, configs):
        ...
        self.cross_conv_season = nn.Sequential(
            Inception_Block_V1(configs.d_model, configs.d_ff, num_kernels=getattr(configs, "num_kernels", 6), stride=(configs.down_sampling_window, 1)),
            nn.GELU(),
            Inception_Block_V1(configs.d_ff, configs.d_model, num_kernels=getattr(configs, "num_kernels", 6)),
        )
...
class MultiScaleTrendCross(nn.Module):
    def __init__(self, configs):
        ...
        self.cross_trans_conv_season = Inception_Trans_Block_V1(
            configs.d_model, configs.d_ff, num_kernels=getattr(configs, "num_kernels", 6), stride=(configs.down_sampling_window, 1)
        )
        self.cross_trans_conv_season_restore = nn.Sequential(
            nn.GELU(), Inception_Block_V1(configs.d_ff, configs.d_model, num_kernels=getattr(configs, "num_kernels", 6))
        )
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.encoder_model = nn.ModuleList([MixerBlock(configs) for _ in range(configs.e_layers)])

        if self.channel_independence:
            self.enc_embedding = DataEmbedding_wo_pos(1, configs.d_model, configs.embed, configs.freq, configs.dropout)
        else:
            self.enc_embedding = DataEmbedding_wo_pos(self.configs.enc_in, configs.d_model, configs.embed, configs.freq, configs.dropout)
        self.revin_layers = torch.nn.ModuleList(
            [Normalize(self.configs.enc_in, affine=True, subtract_last=False) for i in range(configs.down_sampling_layers + 1)]
        )

        self.layer = configs.e_layers

        if self.configs.channel_mixing == 1:
            ...
            self.channel_mixing_attention = AttentionLayer(
                FullAttention(False, attention_dropout=self.configs.dropout, output_attention=self.configs.output_attention), d_time_model, self.configs.n_heads
            )
        ...
        # Add attributes needed for classification and anomaly detection
        if self.task_name == "classification":
            self.act = nn.functional.gelu
            self.dropout = nn.Dropout(configs.dropout)
            self.projection = nn.Linear(configs.d_model * configs.seq_len, configs.num_class, bias=True)
    ...
    def __multi_scale_process_inputs(self, x_enc, x_mark_enc):
        ...
        for i in range(self.configs.down_sampling_layers):
            ...
            if self.configs.down_sampling_method == "conv":
                x_enc_sampling_list.append(x_enc_sampling.reshape(B, N, T // (self.down_sampling_window ** (i + 1))).permute(0, 2, 1))
            else:
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.configs.channel_mixing == 1 and self.channel_independence == 1:
            _, T, D = x_list[-1].size()
