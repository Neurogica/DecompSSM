...
class Dataset_M4(Dataset):
    ...
    def __read_data__(self):
        M4Dataset.initialize(dataset_file=self.root_path)
        if self.flag == "train":
...
class UEAloader(Dataset):
    ...
    def load_single(self, filepath):
        ...
        df = pd.concat(
            (
                pd.DataFrame({col: df.loc[row, col] for col in df.columns}).reset_index(drop=True).set_index(pd.Series(lengths[row, 0] * [row]))
