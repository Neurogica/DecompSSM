...
class Inception_Trans_Block_V1(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, num_kernels=6, init_weight=True):
        ...
        kernels = []
        for i in range(self.num_kernels):
            kernels.append(nn.ConvTranspose2d(in_channels, out_channels, kernel_size=2 * i + 1, padding=i, stride=stride))
        self.kernels = nn.ModuleList(kernels)
        if init_weight:
            self._initialize_weights()
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        if pe is not None:
            ...
            return torch.zeros(x.size(0), x.size(1), self.d_model).to(x.device)
